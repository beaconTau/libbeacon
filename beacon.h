...
#define BN_NUM_BUFFER 1 
#define BN_LEGACY_NUM_BUFFER 4 
...
typedef enum beacon_trigger_type 
{
  ...
  BN_TRIG_PHASED,    //!< triggered by input waveforms
  BN_TRIG_EXT,    //!< triggered by external trigger 
  BN_TRIG_COINC,    //!< triggered by input coincidence trigger 
  BN_TRIG_PPS     //!< triggered by PPS
} beacon_trig_type_t;
...
typedef struct beacon_event
{
  ...
  uint8_t board_id[BN_MAX_BOARDS];     //!< The board number assigned at startup. If the second board_id is zero, that indicates there is no slave device. 
  uint8_t data[BN_MAX_BOARDS][BN_NUM_CHAN][BN_MAX_WAVEFORM_LENGTH]; //!< The waveform data. Only the first buffer_length bytes of each are important. The second array is only filled if there is a slave-device.
} beacon_event_t; 
